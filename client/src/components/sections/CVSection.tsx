...
const EDUCATION = [
  {
    ...
    details: [
      ...
      "Fields: Asset Management, Empirical Asset Pricing, and Fintech",
    ],
  },
  {
          ...
          <div className="bg-white border border-[oklch(0.9_0.008_255)] rounded-lg p-4 hover:border-[oklch(0.38_0.12_255_/_0.3)] hover:shadow-sm transition-all duration-200">
            ...
            <ul className="space-y-1">
              {item.details.map((d, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 text-[0.82rem] text-[oklch(0.4_0.02_255)]"
                >
    ...
    <section id="cv" className="min-h-screen">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h2 className="font-serif text-3xl font-bold text-[oklch(0.2_0.02_255)]">
            Academic CV
          </h2>
        </div>
        <Button
          ...
          onClick={() => window.open(CV_PDF_URL, "_blank", "noopener,noreferrer")}
        >
          <Download size={15} />
          Download PDF
        </Button>
            ...
            <div className="space-y-3">
              {AWARDS.map(({ year, award, detail }) => (
                <div key={year + award} className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[0.85rem] font-medium text-[oklch(0.2_0.02_255)] leading-snug">
                      {award}
                    </p>
                    <p className="text-[0.75rem] text-[oklch(0.52_0.015_255)]">
                      {detail}
                    </p>
                  </div>
                </div>
